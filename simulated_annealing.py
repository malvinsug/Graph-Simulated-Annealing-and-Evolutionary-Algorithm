...
while temperature >1:
    ...
    ratio = math.exp((current_distance-new_distance)/temperature)
    #print("{0} and {1} \n {2} and {3}".format(new_distance,current_distance,s,ratio))
    #print("new_distance< current_distance or s<ratio is {0}".format((new_distance < current_distance) or (s<ratio)))
    if (new_distance < current_distance) or (random.random()<=ratio):
        [trip,x,y,current_distance] = [new_trip,new_x,new_y,new_distance]
        #print("{0}. solution has a distance : {1}".format(counter,current_distance))
